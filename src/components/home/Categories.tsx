...
export default function Categories() {
  const topExpensiveProducts = [...products]
    .filter(p => p.id !== "vapor-maker-2022")
    ...
    <section className="py-24 bg-brand-bg border-y border-white/10">
      <div className="max-w-[1400px] mx-auto">
        <div className="px-12 py-12 flex flex-col md:flex-row justify-between items-end gap-6">
          <div>
            <span className="font-mono text-[10px] uppercase tracking-[0.4em] text-brand-secondary mb-3 block">Premium Inventory</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-white/10 border-t border-white/10">
          {topExpensiveProducts.map((prod, i) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="group relative h-[500px] bg-brand-bg overflow-hidden flex flex-col justify-between p-10 hover:bg-[#111] transition-colors duration-500 cursor-crosshair"
            >
              <Link to={`/shop/${prod.id}`} className="absolute inset-0 z-30 cursor-crosshair" />
              <div className="flex justify-between items-start z-20 pointer-events-none">
                ...
                <div className="w-10 h-10 border border-white/20 rounded-full flex items-center justify-center group-hover:border-brand-secondary transition-colors duration-500">
                  <Target size={16} className="text-brand-secondary scale-0 group-hover:scale-100 transition-transform duration-500" />
                </div>
              </div>
